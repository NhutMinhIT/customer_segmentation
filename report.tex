...
Before training the clustering models, it is crucial to address potential data leakage issues that could compromise model validity and customer privacy. In the context of customer segmentation, several types of information leakage must be considered:

\textbf{Feature Engineering Leakage}

When creating the derived feature \texttt{Spending\_Score\_Per\_Income}, care must be taken to ensure that no future information or target-related data influences the feature construction. Since this is an unsupervised learning task, there is no explicit target variable; however, the derived feature must be computed using only information that would be available at the time of customer classification in a real deployment scenario.

\textbf{Data Preprocessing Leakage}

A common source of data leakage occurs during the standardization process. The statistics used for scaling (mean $\mu$ and standard deviation $\sigma$) must be computed using only the training data. In this project, since we are performing exploratory clustering on the entire dataset, the scaling parameters are calculated from the complete dataset. However, in a production environment where new customers would be classified using the trained model, the scaling parameters from the original training set must be preserved and applied to new data points.

The standardization formula applied is:
\[
z = \frac{x - \mu_{\text{train}}}{\sigma_{\text{train}}}
\]
where $\mu_{\text{train}}$ and $\sigma_{\text{train}}$ are computed exclusively from the training data.

\textbf{Temporal Information Leakage}

Although the current dataset does not contain temporal features, in real-world customer segmentation scenarios, care must be taken to ensure that future information (e.g., future purchase behavior, seasonal trends) does not leak into the features used for current customer classification.

\textbf{Privacy Preservation}

Customer segmentation involves processing sensitive personal and financial information. To mitigate privacy risks:

\begin{itemize}
    \item Customer identifiers (\texttt{CustomerID}) are removed from the clustering process.
    \item The model focuses on behavioral patterns rather than individual identification.
    \item Clustering results should be aggregated at the segment level to prevent individual customer re-identification.
    \item In production deployments, additional privacy-preserving techniques such as differential privacy or k-anonymity may be required.
\end{itemize}

\textbf{Validation Methodology}

Since clustering is unsupervised, traditional train-test splits are not directly applicable. However, to validate model stability and prevent overfitting to specific data characteristics:

\begin{itemize}
    \item Multiple random initializations are used for K-Means to ensure convergence consistency.
    \item Cluster validity is assessed using multiple internal validation metrics.
    \item Cross-validation techniques such as stability analysis across data subsamples could be employed in more extensive studies.
\end{itemize}

By addressing these leakage concerns, the clustering model maintains its integrity for both analytical insights and potential production deployment while protecting customer privacy throughout the segmentation process.
...
Figure~\ref{fig:optk} shows both charts. The WCSS curve exhibits a clear elbow at $K = 5$. The Silhouette Score reaches its global maximum at $K = 7$ (score = 0.4234); however, the improvement over $K = 5$ (score = 0.4105) is marginal, and the scatter plot in Figure~\ref{fig:corr} visually reveals five natural groupings in the Income--Spending Score space. Considering both the Elbow criterion and domain interpretability, $K = 5$ is selected for both clustering models.
...
\begin{figure}[H]
    ...
    \caption{Hierarchical clustering result: Income vs.\ Spending Score (left) and PCA 2D projection (right). PCA axes are identical to Figure~\ref{fig:kmeanspca} (PC1: 49.5\%, PC2: 28.9\%)}
...
Based on the clustering results, six business segments were identified from five K-Means clusters:
...
\begin{itemize}
    ...
    \item \textbf{Middle Income -- Middle Spending, Mature (Standard -- Older)}: Core group of older customers (mean age $\approx 56$, $n = 58$); suitable for loyalty card programs and convenience-oriented promotions.
    \item \textbf{Middle Income -- Middle Spending, Young (Standard -- Younger)}: Core group of younger customers (mean age $\approx 28$, $n = 46$); suitable for digital engagement, upsell campaigns, and membership programs.
\end{itemize}

\noindent\textit{Note: Although both Standard segments share similar income and spending levels, they form two distinct clusters primarily differentiated by age, and are therefore treated separately in the segmentation output.}
...
Two clustering algorithms were evaluated: K-Means and Hierarchical Clustering (Agglomerative, Ward linkage), both configured at $K = 5$ clusters. The value $K = 5$ was selected based on the Elbow Method (clear inflection point at $K = 5$) and visual inspection of natural groupings in the scatter plot, despite the Silhouette Score reaching its global maximum at $K = 7$ (0.4234 vs.\ 0.4105 at $K = 5$) -- a marginal gain that was deemed insufficient to justify the additional segmentation complexity.
...
\begin{itemize}
    ...
    \item Six distinct customer segments were identified from five K-Means clusters: VIP, Impulsive, Conservative, Cautious, Standard (Older), and Standard (Younger) -- with the two Standard sub-groups differentiated primarily by age (mean age $\approx 56$ vs.\ $\approx 28$).
\end{itemize}
...
The six identified segments (derived from five K-Means clusters) provide actionable insights for the mall management:
...
\begin{itemize}
    ...
    \item The Standard (Older) segment, comprising the largest single cluster ($n = 58$), benefits from convenience-oriented loyalty programs tailored to mature shoppers.
    \item The Standard (Younger) segment ($n = 46$) is well-suited for digital engagement channels, upsell campaigns, and general membership programs.
\end{itemize}
